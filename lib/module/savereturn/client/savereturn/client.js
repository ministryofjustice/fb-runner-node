...
class FBSaveReturnClient extends FBJWTClient {
   ...
   *
   **/
  constructor (serviceSecret, serviceSlug, saveReturnUrl, encodedPrivateKey) {
    super(serviceSecret, serviceSlug, saveReturnUrl, SaveReturnClientError, encodedPrivateKey)
  }
  ...
  async sendPost (args, logger) {
    ...
    Object.keys(payload).forEach(key => {
      ...
      if (key.startsWith('encrypted_')) {
        value = this.encrypt(this.serviceSecret, value, undefined)
      }
  ...
  async createSetupEmailToken (userId, userToken, email, duration, logger) {
    ...
    const { token } = await this.sendPost({
      url: '/setup/email/add',
      payload: {
  ...
  async createSetupMobileCode (userId, userToken, email, mobile, duration, logger) {
    ...
    const { code } = await this.sendPost({
  ...
  async validateSetupEmailToken (emailToken, passphrase, logger) {
    ...
    const { details } = await this.sendPost({
