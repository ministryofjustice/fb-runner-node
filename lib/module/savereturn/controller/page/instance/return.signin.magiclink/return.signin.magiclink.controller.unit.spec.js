...
test('When a user with 2fa enabled visits a valid magic link', async t => {
  ...
  t.equal(instance.redirect, 'return.signin.code.sent', 'it should update the page instance redirect property with the expected value')
...
test('When a user visits a valid magic link that is not valid', async t => {
  ...
  t.deepEqual(handleValidationErrorStub.getCall(0).args, [error, 'return.signin'], 'it should call handleValidationError with the expected args')
