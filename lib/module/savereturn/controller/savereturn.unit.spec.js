...
const emailClientStub = stub(emailClient, 'send')
const smsClient = {
  send: () => {}
}
const smsClientStub = stub(smsClient, 'send')
...
const savereturn = proxyquire('./savereturn', {
  '~/fb-runner-node/client/email/email': emailClient,
  '~/fb-runner-node/client/sms/sms': smsClient,
...
test('When requiring the savereturn module', t => {
  const expectedClient = require('~/fb-runner-node/module/savereturn/client/savereturn/savereturn')
