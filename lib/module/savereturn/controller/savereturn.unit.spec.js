...
test('When getting messages', async t => {
  ...
  const message = getMessage('email.test', userData, {personalisation_foo: 'pbar'}, {data_foo: 'dbar'})
...
test('When sending emails', async t => {
  ...
  await sendEmail('email.test', userData, {personalisation_foo: 'pbar'}, {data_foo: 'dbar'})
  ...
  t.deepEqual(emailClientStub.getCall(0).args, [{_id: 'email.test'}, {logger: userData.logger}], 'it should call emailClient.sendMessage with the expected args')
...
test('When sending sms messages', async t => {
  ...
  await sendSMS('sms.test', userData, {personalisation_foo: 'pbar'}, {data_foo: 'dbar'})
  ...
  t.deepEqual(smsClientStub.getCall(0).args, [{_id: 'sms.test'}, {logger: userData.logger}], 'it should call smsClient.sendMessage with the expected args')
