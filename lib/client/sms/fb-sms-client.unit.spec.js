...
test('When decrypting the user’s data', async t => {
  const decryptedPayload = smsClient.decrypt(userToken, encryptedPayload)
...
test('When decrypting invalid data', async t => {
  let invalidPayload
  try {
    t.throws(invalidPayload = smsClient.decrypt(userToken, 'invalid'))
  } catch (e) {
    t.equal(e.name, 'FBSMSClientError', 'it should return an error object of the correct type')
...
test('When encrypting the user’s data', async t => {
  const encryptedPayload = smsClient.encrypt(userToken, payload)
  const decryptedPayload = smsClient.decrypt(userToken, encryptedPayload)
  ...
  const encryptedPayloadAgain = smsClient.encrypt(userToken, payload)
...
test('When sending a message', async t => {
  ...
  const logger = {error: () => {}}
  const responseBody = await smsClient.sendMessage(payload, {logger})
