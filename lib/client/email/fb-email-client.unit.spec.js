...
const createEndpointUrl = `${emailUrl}/service/${serviceSlug}/user/${userId}`
...
const testInstantiation = (t, params, expectedCode, expectedMessage) => {
  let failedClient
  try {
    t.throws(failedClient = new FBEmailClient(...params))
  } catch (e) {
    t.equal(e.name, 'FBEmailClientError', 'it should return an error of the correct type')
...
test('When instantiating email client without a service token', t => {
  testInstantiation(t, [serviceSecret], 'ENOSERVICETOKEN', 'No service token passed to client')
})

test('When instantiating email client without a service slug', t => {
  testInstantiation(t, [serviceSecret, serviceToken], 'ENOSERVICESLUG', 'No service slug passed to client')
})

test('When instantiating email client without a email url', t => {
  testInstantiation(t, [serviceSecret, serviceToken, serviceSlug], 'ENOMICROSERVICEURL', 'No microservice url passed to client')
})

test('When instantiating email client without a service secret', t => {
  testInstantiation(t, [undefined, serviceToken, serviceSlug, emailUrl], 'ENOSERVICESECRET', 'No service secret passed to client')
})

// Set up a client to test the methods
const emailClient = new FBEmailClient(serviceSecret, serviceToken, serviceSlug, emailUrl)

// Error class
test('When throwing errors', t => {
  t.equal(emailClient.ErrorClass.name, 'FBEmailClientError', 'it should use the client\'s error class')
...
test('When asking for endpoint urls', t => {
  const getUrl =
  emailClient.createEndpointUrl('/service/:serviceSlug/user/:userId', {serviceSlug, userId})
...
test('When generating json web token', async t => {
  ...
  const accessToken = emailClient.generateAccessToken({payload: 'testPayload'})
...
test('When decrypting the user’s data', async t => {
  const decryptedPayload = emailClient.decrypt(userToken, encryptedPayload)
...
test('When decrypting invalid data', async t => {
  let invalidPayload
  try {
    t.throws(invalidPayload = emailClient.decrypt(userToken, 'invalid'))
  } catch (e) {
    t.equal(e.name, 'FBEmailClientError', 'it should return an error object of the correct type')
...
test('When encrypting the user’s data', async t => {
  const encryptedPayload = emailClient.encrypt(userToken, payload)
  const decryptedPayload = emailClient.decrypt(userToken, encryptedPayload)
  ...
  const encryptedPayloadAgain = emailClient.encrypt(userToken, payload)
...
test('When sending a message', async t => {
  const sendPostStub = stub(emailClient, 'sendPost')
  ...
  const logger = {error: () => {}}
  const responseBody = await emailClient.sendMessage(payload, {logger})

  const callArgs = sendPostStub.getCall(0).args
  t.equal(callArgs[0].url, '/email', 'it should pass the correct url pattern')
  t.deepEqual(callArgs[0].payload, payload, 'it should pass the correct encrypted payload')
...
test('When sending a message results in an error', async t => {
  ...
  try {
    t.throw(await emailClient.sendMessage({}, {logger}))
  } catch (e) {
