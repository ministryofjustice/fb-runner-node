...
test('When generating json web token', async t => {
  ...
  const accessToken = emailClient.generateAccessToken({payload: 'testPayload'}, serviceToken, 'HS256')
...
test('When sending a message', async t => {
  ...
  const logger = {error: () => {}}
  const responseBody = await emailClient.sendMessage(message, {sendOptionX: 'x'}, logger)
