...
const routesMetadata = require('./middleware/routes-metadata')
...
const jsonLoader = require('@ministryofjustice/fb-runtime-node/lib/json-loader')
const mergeInstances = require('@ministryofjustice/fb-runtime-node/lib/merge-instances')

const loadData = (siteData) => {
  return jsonLoader.load(siteData).then(loadedInstances => {
    return mergeInstances.merge(loadedInstances)
  })
}
...
disallow: /public`
  ...
  // Local values
  app.use(locals(ENV, ASSET_PATH, CONSTANTS))
  ...
  if (options.siteData) {
    app.use(routesMetadata(options.siteData))
  }
...
const start = (options = {}) => {
  const siteData = [{
    source: 'site',
    path: path.resolve(SITEDATAPATH)
  }]
  return loadData(siteData)
    .then(loadedData => {
      options.siteData = loadedData
