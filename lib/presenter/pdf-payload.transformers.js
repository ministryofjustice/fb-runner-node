...
const hasGroup = (group) => ({component, groupBy = component}) => group === groupBy

const aggregateGroups = (accumulator = [], {component, groupBy = component, ...answer} = {}, i, answers) => accumulator.some(hasGroup(groupBy)) ? accumulator : accumulator.concat(transformGroup({...answer, groupBy}, answers.filter(hasGroup(groupBy))))
