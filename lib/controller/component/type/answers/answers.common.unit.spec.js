...
test('Populating the answer bucket for a MOJ file upload component when a file has not been uploaded', (t) => {
  getUrlStub.reset()
  getDisplayValueStub.reset()
  striptagsStub.reset()
  ...
  const questionInstance = {name: 'component name', label: 'component label', _type: 'mojFileupload'}
  const componentInstance = {hideChangeAction: false}
  const currentPage = {params: 'params'}
  const currentInstance = {_id: 'page id'}
  const userDataStub = {
