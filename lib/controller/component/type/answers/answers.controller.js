...
answersComponent.preUpdateContents = (componentInstance, userData, pageInstance) => {
    // TODO: UH: Why the JSON.parse/JSON.stringify?
  const hideChangeAction = JSON.parse(JSON.stringify(componentInstance)).hideChangeAction
  const onlyShowCompletedAnswers = JSON.parse(JSON.stringify(componentInstance)).onlyShowCompletedAnswers
  ...
  while (nextPage) {
    ...
    if (!stepInstance.$skipPage) {
      ...
      names.forEach(nameInstance => {
        ...
        question = format(question, userData.getUserData('input'), {lang: userData.contentLang})

        let answer = name ? userData.getUserDataProperty(name, null, 'input') : ''
        ...
        if (nameInstance._type === 'checkboxes') {
          if (nameInstance.items) {
            const values = nameInstance.items.filter(item => {
              ...
              return itemValue === userData.getUserDataProperty(item.name, null, 'input')
            }).map(item => {
