...
function getAnswerBucketItemForMOJUpload (questionInstance, {hideChangeAction = false}, currentPage, currentInstance, userData, pageInstance) {
  const {_id: currentInstanceId} = currentInstance
  const {name: questionName} = questionInstance
  const value = userData.getUserDataProperty(questionName) || []
...
function populateMOJUploadAnswerBucket (answerBucket, ...args) {
  const answerBucketItem = getAnswerBucketItemForMOJUpload(...args)
...
function populateAnswerBucket (answerBucket, questionInstance, ...args) {
  ...
  switch (_type) {
    ...
    case 'fileupload':
      ...
      break
    case 'mojUpload':
      populateMOJUploadAnswerBucket(answerBucket, questionInstance, ...args)
      break
    default:
