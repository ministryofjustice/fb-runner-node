...
function createAnswerBucketItem (page, component, key, value, changeAction = {}, groupBy) {
  return {
    page,
    component,
    key,
...
function getAnswerBucketItemsForCheckboxes (questionInstance, ...args) {
  const {
    items = []
  } = questionInstance

  return items
    .filter(({value, label}) => !isUndefinedOrNull(value) || !isUndefinedOrNull(label))
    .map((questionInstanceItem) => getAnswerBucketItemForCheckbox(questionInstanceItem, questionInstance, ...args))
}
