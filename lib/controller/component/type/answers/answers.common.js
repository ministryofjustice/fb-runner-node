...
function getAnswerBucketItemForMOJFileUpload (questionInstance, {hideChangeAction = false}, currentPage, currentInstance, userData, pageInstance) {
  const {_id: currentInstanceId} = currentInstance
  const {name: questionName} = questionInstance
  const value = userData.getUserDataProperty(questionName) || []
...
function populateMOJFileUploadAnswerBucket (answerBucket, ...args) {
  const answerBucketItem = getAnswerBucketItemForMOJFileUpload(...args)
...
function populateAnswerBucket (answerBucket, questionInstance, ...args) {
  ...
  switch (_type) {
    ...
    case 'fileupload':
      ...
      break
    case 'mojFileupload':
      populateMOJFileUploadAnswerBucket(answerBucket, questionInstance, ...args)
      break
    default:
