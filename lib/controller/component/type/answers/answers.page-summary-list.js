...
const filterAnswers = (answer) => Reflect.has(answer, 'heading') || answer.answers.length
...
function getPageSummaryListAnswers (componentInstance, userData, pageInstance) {
  const {
    ...
    while (currentPage) {
      ...
      if (!currentInstance.$skipPage) {
        ...
        questionInstances.forEach(questionInstance => {
          ...
          /**
           *  Populate the answer bucket with questions and answers by component type
           */
          populateAnswerBucket(
            answerBucket,
            questionInstance,
