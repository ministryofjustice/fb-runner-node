...
test('when invoking the answers component', t => {
  const {
    jsonpathStub,
    formatStub,
    pageStub,
  ...
  const nameInstancesPage2 = [{
    ...
    items: [{
      value: 'Value for checkbox item 1'
    }, {
      value: undefined
    }, {
      value: 'Value for checkbox item 3'
    }]
  }, {
    multiline: true,
    _type: 'fileupload',
  ...
  userDataStub.getUserDataInputProperty.onCall(1).returns('the answer for a certain question')

  userDataStub.getUserDataInputProperty.onCall(4).returns('Value for checkbox item 1')
  userDataStub.getUserDataInputProperty.onCall(6).returns('Value for checkbox item 3')

  userDataStub.getUserDataInputProperty.onCall(7).returns('Answer for fileupload')
  ...
  const result = preUpdateContents(
    componentInstance,
    userDataStub,
    pageInstance
  )
  ...
  for (const method of ['skipPage', 'setControlNames', 'setRepeatable']) {
    ...
    t.equal(callCount, 3, `${method} was called twice for two pages`)
  }
