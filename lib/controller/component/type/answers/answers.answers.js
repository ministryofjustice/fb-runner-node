...
function filterAnswers (answer) {
  return !answer.answers || answer.answers.length
}
...
function getAnswers ({hideChangeAction = false, onlyShowCompletedAnswers = false}, userData, pageInstance) {
  const visitedPages = userData.getUserDataProperty('visited', [], 'visited')

  const formatQuestion = (nameInstance) => {
    const lang = userData.contentLang

    if (lang === 'cy') {
      return nameInstance['label:cy'] || nameInstance['legend:cy']
    }

    return nameInstance.label || nameInstance.legend
  }

  const formatHeading = (stepInstance) => {
    const lang = userData.contentLang

    if (lang === 'cy') {
      return stepInstance['heading:cy']
    }

    return stepInstance.heading
  }
  ...
  while (nextPage) {
    ...
    if (!stepInstance.$skipPage) {
      ...
      if (names.length) {
        ...
        if (pageHeading) {
          pageHeading = formatHeading(stepInstance)

          const pageAnswers = {
            heading: pageHeading,
            repeatable: stepInstance.repeatable || false,
            level: level++
          }
