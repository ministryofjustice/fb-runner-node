...
module.exports = class SummaryController extends CommonController {
  ...
  async postValidation (pageInstance, userData) {
    ...
    if (submitterClient) {
      ...
      this.attachEmailSubmissions(submissionId, userData, submissions)
      this.attachJsonSubmission(submissions, userData)
      this.attachCsvSubmission(submissions, userData)

      const formData = this.generatePDFPayload(userData, submissionId)
