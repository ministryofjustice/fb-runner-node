...
module.exports = class SummaryController extends CommonController {
  ...
  async attachEmailSubmissions (submissionId, userData, submissions) {
    function formatEmail (str, markdown = false) {
      try {
        ...
        log(scopedUserData) // NOT FOR PROD
    ...
    const from = defaultEmailAddress ? formatEmail(defaultEmailAddress) : '"Form Builder" <form-builder-team@digital.justice.gov.uk>'
    ...
    log(unscopedUserData) // NOT FOR PROD
    ...
    log(uploadedFiles) // NOT FOR PROD
  ...
  async attachJsonSubmission (submissions, userData) {
    ...
    const outputData = userData.getOutputData()
    const uploadedFiles = userData.uploadedFiles()

    log(outputData) // NOT FOR PROD
    log(uploadedFiles) // NOT FOR PROD
