...
module.exports = class SummaryController extends CommonController {
  ...
  async attachCsvSubmission (submissions, userData) {
    ...
    let userAnswer = userData.getOutputData()
    const heading = getInstanceProperty('service', 'pdfHeading')
    const subHeading = getInstanceProperty('service', 'pdfSubHeading')
    const answersForCsv = pdfPayload(
      await submissionDataWithLabels(
        userAnswer.submissionId,
        heading,
        subHeading,
        userData
      )
    )
    if (answersForCsv && answersForCsv.sections) {
      answersForCsv.sections.map((section) => {
        section.questions.filter((question) => {
          // Ignore uploads on CSV data.
          return !Array.isArray(question.answer)
        }).map((question) => {
          if (question.human_value === 'Not answered') {
            submission.user_answers[question.key] = null
          } else {
            submission.user_answers[question.key] = question.human_value
          }
        })
      })
    }

    submission.user_answers = removeUploadComponentValuesForCsv({
