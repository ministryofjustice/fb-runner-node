...
SummaryController.preFlight = async (instance, userData) => {
  ...
  if (performsSubmit) {
    ...
    reachedPageInstance = await getRedirectForNextPage(reachedPageInstance, userData)
...
SummaryController.attachEmailSubmissions = async (submissionId, userData, submissions) => {
  ...
  const teamSubmission = generateEmail('team', from, subject, submissionId, true, true, emailBody)
  ...
  if (emailUserInputName) {
    ...
    if (userEmail) {
      ...
      const userSubmission = generateEmail('user', from, subject, submissionId, includePdf, false, emailBody)
