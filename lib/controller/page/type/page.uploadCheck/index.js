  ...
  async postValidation (pageInstance, userData) {
    ...
    if (this.isUploadAccepted(userData)) {
      ...
      if (this.isUploadRejected(userData)) {
        ...
        const checkInstance = getInstance(pageId)
        let content = '<h2 class="govuk-error-summary__title" id="error-summary-title">There is a problem</h2><ul class="govuk-list govuk-error-summary__list"><li><a href="#upload-component-decision-0">Enter an answer for '

        if (checkInstance && checkInstance.components && checkInstance.components.length > 1) {
          content += 'Are these uploads correct?</a></li></ul>'
        } else {
          content += 'Is this upload correct?</a></li></ul>'
        }

        userData.setFlashMessage({
          type: 'error.required.radios',
          html: content
        })
      }
    }
