...
function isUploadComponentStep (pageInstance) {
  return (
    isUploadCheckPage(pageInstance) || isUploadSummaryPage(pageInstance)
  )
}

function getPageWithUploadComponentFor ({ _parent } = {}) {
  let candidateInstance = getInstance(_parent)

  /*
   *  Find the upload page
   */
  while (!hasUploadComponents(candidateInstance)) {
    const { _parent } = candidateInstance
    candidateInstance = getInstance(_parent)
  }

  /*
   *  Found the upload page -- return it
   */
...
function getNextPage (currentPage, userData, recurse) {
  const { _id } = currentPage
  const currentInstance = getInstance(_id)

  /*
   *  If this page has upload components, ensure that the user proceeds to the check step
   */
  if (hasUploadComponents(currentInstance)) {
    /*
     *  Spread the items to a new array
     */
    const steps = [
      ...getInstanceProperty(_id, 'steps', [])
    ]
    const step = steps.shift()

    if (step) {
      let candidateInstance = getInstance(step)

      while (!isUploadCheckPage(candidateInstance)) {
        const step = steps.shift()

        if (step) {
          candidateInstance = getInstance(step)
        } else {
          break
        }
      }
   ...
   *  (if it is present)
   */
  if (isUploadCheckPage(currentInstance)) {
    /*
     *  Spread the items to a new array
     */
    const steps = [
    ...
    if (step) {
      ...
      while (!isUploadSummaryPage(candidateInstance)) {
        const step = steps.shift()
...
function checkPageIdParams (alpha, omega) {
  if (!omega) {
    return false
  }
...
/*
 *  This is a weird function. I'm not sure how to refactor it (yet) so I'm commenting it up. Hello
 *
 *  It does one or both of two things:
 *
 *  First, it asks whether or not the current page is a step of an upload journey. If it is: the previous
 *  page is the parent of the step
 *
 *  Second, and if it is not a step of an upload journey, it rewinds and fast-forwards again up to
 *  the previous page
 *
 *  I've tried to do some renaming of variables to make their content a bit clearer. Hope it helps
 */
function getPreviousPage (currentPage, userData) {
  let { _id } = currentPage

  /*
   *  No _id, no previous page
   */
  if (_id) {
    let currentInstance = getInstance(_id)

    /*
     *  Determine whether the current page is an upload check or summary step
     */
    if (isUploadComponentStep(currentInstance)) {
      /*
       *  It is. Get the page with the upload component
       */
      return getPageWithUploadComponentFor(currentInstance)
    }

    /*
     *  Rewind all the way back to the start
     */
    if (Reflect.has(currentInstance, '_parent')) {
      _id = Reflect.get(currentInstance, '_parent')
      currentInstance = getInstance(_id)

      while (Reflect.has(currentInstance, '_parent')) {
        _id = Reflect.get(currentInstance, '_parent')
        currentInstance = getInstance(_id)
      }

      /*
       *  Start with something. It isn't an instance (but it will get swapped for an instance)
       */
      let candidatePage = { _id }

      while (candidatePage) {
        /*
         *  Fast forward through each page ...
         */
        const nextInstance = getNextPage(candidatePage, userData, true)

        if (nextInstance) {
          /*
           *  Is this the previous page?
           */
          if (checkPageIdParams(currentPage, nextInstance)) {
            /*
             *  Determine whether the current page is an upload check or summary step
             */
            const { _id } = candidatePage
            const candidateInstance = getInstance(_id)

            if (isUploadComponentStep(candidateInstance)) {
              /*
               *  It is. Get the page with the upload component
               */
              return getPageWithUploadComponentFor(candidateInstance)
            }

            /*
             *  It is neither a check nor a summary step -- return the current value of 'candidatePage'
             */
        ...
        /*
         *  The value of `nextInstance` may be undefined which will halt the while loop
         *  to fall through and implicitly return undefined
         */
        candidatePage = nextInstance
      }
    }
  }
}
