...
const getNextPage = (input, userData) => {
  ...
  if (steps[0]) {
    for (let index = 0; index < steps.length; index++) {
      ...
      const repeatable = getInstanceProperty(step, 'repeatable')
      if (repeatable) {
        const nextRepeatable = checkNextRepeatable(step, params, userData)
        if (nextRepeatable) {
        // FOUND
          return nextRepeatable
        }
        ...
        const updatedParams = updateParams(step, params, 'next')
        if (checkPageVisibility(step, userData)) {
          // FOUND
          return {_id: step, params: updatedParams}
        }
      }
    }
  }

  const nextPage = checkCurrentPage(input, userData)
  if (!nextPage) {
    return
  }
  if (checkPageVisibility(nextPage._id || nextPage, userData)) {
    return nextPage._id ? nextPage : {_id: nextPage}
  } else {
