...
const getRouteUrl = (pagesMethods, route, params, lang) => {
...
const getNextUrl = (input, userData) => {
  ...
  if (nextpage) {
    return userData.pagesMethods.getUrl(nextpage._id, nextpage.params, userData.contentLang)
      ...
      for (let index = 0; index < explicitNextPage.length; index++) {
        ...
        if (typeof nextPage === 'object') {
          if (nextPage.condition !== true) {
            nextPage = evaluate(nextPage.condition, userData.getAllData(), params) ? nextPage.page : undefined
          }
        }
...
const getPreviousUrl = (inputPage, userData) => {
  ...
  if (previouspage) {
    return userData.pagesMethods.getUrl(previouspage._id, previouspage.params, userData.contentLang)
