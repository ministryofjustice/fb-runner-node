...
function getPreviousPage (currentPage, userData) {
  ...
  if (_id) {
    ...
    /*
     *  Determine whether the current page is a check or summary step
     */
    if (isUploadCheckPage(currentInstance) || isUploadSummaryPage(currentInstance)) {
      /*
       *  It is
       */
      const { _parent } = currentInstance
     ...
     */
    let { _parent } = currentInstance

    while (Reflect.has(currentInstance, '_parent')) {
      _parent = Reflect.get(currentInstance, '_parent')
      currentInstance = getInstance(_parent)
    }

    /*
     *  Start with something. It isn't an instance (but it will get swapped for an instance)
     */
    let candidatePage = { _id: _parent }
    ...
    while (candidatePage) {
      ...
      if (nextInstance) {
        ...
        if (checkPageIdParams(currentPage, nextInstance)) {
          /*
           *  Determine whether the previous page is a check or summary step
           */
          const { _id } = candidatePage
          let candidateInstance = getInstance(_id)

          if (isUploadCheckPage(candidateInstance) || isUploadSummaryPage(candidateInstance)) {
            /*
             *  It is
             */
            const { _parent } = candidateInstance
