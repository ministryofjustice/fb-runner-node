...
test('When considering which page to go to', t => {
  ...
  // console.log(serviceData.getInstance('x'))

  let userData = getUserDataMethods({input: {}, count: {}})
  let userDataDefined = getUserDataMethods({input: {check: true}, count: {v: 2, w: 3}})
  let userDataSkipB = getUserDataMethods({input: {skipB: true}, count: {}})
  ...
  const tests = [
    ...
    // {input: {_id: 'v', params: {v: 1}}, userData, output: {_id: 'w', params: {w: 1}}}
    {input: {_id: 'v', params: {v: 1}}, userData: userDataDefined, output: {_id: 'v', params: {v: 2}}}
    // {input: {_id: 'w', params: {w: 1}}, userData, output: {_id: 'w', params: {w: 2}}},
    // {input: {_id: 'w', params: {w: 2}}, userData, output: {_id: 'w', params: {w: 3}}}

    // ,{input: {_id: 'w', params: {w: 3}}, userData, output: {_id: 'x', params: {x: 1}}}

    // {input: {_id: 'x', params: {x: 1}}, userData, output: {_id: 'y', params: {x: 1, y: 1}}},
    // {input: {_id: 'y'}, userData, output: {_id: 'z', params: {z: 1}}},
    // {input: {_id: 'z'}, userData, output: {_id: 'e'}},
    ...
    // {input: {_id: 'b2'}, userData: userDataDefined, output: {_id: 'd'}, skip: 'c'}
  ]
  ...
  tests.forEach(nextTest => {
    ...
    if (input) {
      if (output) {
        t.deepEqual(getNextPage(input, userData), output, `it should go forward from ${input._id} to ${output._id}${skipMessage}`)
      } else {
        t.deepEqual(getNextPage(input, userData), output, `it should not go forward from ${input._id}`)
      }
    }
    // if (output) {
