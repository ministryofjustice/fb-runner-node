...
test('When considering which page to go to', t => {
  ...
  const workingTests = [
    ...
    {input: {_id: 'y', params: {x: 1, y: 1}}, userData, output: {_id: 'z', params: {x: 1, y: 1, z: 1}}},
    {input: {_id: 'z', params: {x: 1, y: 1, z: 1}}, userData, output: {_id: 'e'}},
  ...
  tests.forEach(nextTest => {
    ...
    if (input) {
      if (output) {
        ...
        t.deepEqual(actualOutput, output, `it should go forward from ${input._id} to ${output._id}${skipMessage}`)
      } else {
