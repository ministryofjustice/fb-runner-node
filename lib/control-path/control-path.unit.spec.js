...
test('When getting the value of a path', t => {
  ...
  t.deepEqual(get(testData, 'a[b]'), [{c: true}, {c: false}], 'it should handle paths containing brackets without quotes')
  t.deepEqual(get(testData, 'a["b"]'), [{c: true}, {c: false}], 'it should handle paths containing brackets with double quotes')
  t.deepEqual(get(testData, 'a[\'b\']'), [{c: true}, {c: false}], 'it should handle paths containing brackets with single quotes')
  t.deepEqual(get(testData, 'a[b][0]'), {c: true}, 'it should handle array items - paths containing brackets with numeric values')
  t.deepEqual(get(testData, 'a[b][0][c]'), true, 'it should handle paths containing properties of array items')
  t.deepEqual(get(testData, 'a[d]'), undefined, 'it should handle paths that evaluate to undefined')
  t.deepEqual(get(testData, 'a[d]', 'default value'), 'default value', 'it should return a default value when requested for undefined values')
  t.deepEqual(get(testData, 'a[empty][string]', 'default value'), '', 'it should not return a default value when result is an empty string')
  t.deepEqual(get(testData, 'a[empty][number]', 'default value'), 0, 'it should not return a default value when result is zero')
  t.end()
})
...
test('When unsetting the value of a path', t => {
  ...
  t.equal(unset(testDataB, 'a[z][z]'), true, 'it should handle paths that evaluate to undefined')
