...
test('When the service has no config.modules.json and no modules are enabled', t => {
  ...
  const serviceSources = configureModules([{source: 'a', sourcePath: 'source-path-a'}], 'missing/service_config_path', 'runner_dir')

  t.deepEqual(serviceSources, [{source: 'a', sourcePath: 'source-path-a'}], 'it should return service sources unchanged')
...
test('When the service has a config.modules.json but no modules are recorded in it', t => {
  ...
  const serviceSources = configureModules([{source: 'a', sourcePath: 'source-path-a'}], 'exists/service_config_path', 'runner_dir')
...
test('When the service has a config.modules.json and all the modules are recorded in it', t => {
  ...
  const serviceSources = configureModules([{source: 'a', sourcePath: 'source-path-a'}], 'registered/service_config_path', 'runner_dir')

  t.deepEqual(serviceSources, [{source: 'a', sourcePath: 'source-path-a'}, {source: 'module:module_a', sourcePath: 'runner_dir/lib/module/module_a', module: 'module_a'}], 'it should add enabled modules to service sources')
...
test('When the service has a config.modules.json recording modules that do not exist', t => {
  ...
  const expectedModulesConfigContent = deepClone(defaultModulesConfigContent)
  const moduleAConfig = expectedModulesConfigContent.modules[0]
  moduleAConfig.enabled = 'on'
  moduleAConfig.title = 'Module A'

  const serviceSources = configureModules([{source: 'a', sourcePath: 'source-path-a'}], 'additional/service_config_path/config.modules.json', 'runner_dir')

  t.deepEqual(serviceSources, [{source: 'a', sourcePath: 'source-path-a'}, {source: 'module:module_a', sourcePath: 'runner_dir/lib/module/module_a', module: 'module_a'}], 'it should not add modules to service sources if the modules do not exist even if enabled')
  t.ok(writeFileSyncStub.calledOnce, 'it should update config.modules.json')
