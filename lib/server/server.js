...
`
  ...
  initControllers()
  componentSources.reverse().forEach(dataSource => {
...
const start = async (options = {}) => {
  const {metadataSources, specifications} = await prepareServiceSources()
  setServiceSources(metadataSources)

  const schemas = await schemaUtils(specifications).load()
