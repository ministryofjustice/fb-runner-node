...
disallow: /public`
  ...
  if (serviceToken) {
    app.use((req, res, next) => {
      const makeDigest = (userId, userToken, serviceToken) => {
      ...
      let userValues = {}
      let formbuilderCookieValue = req.cookies._formbuilder
      try {
        if (formbuilderCookieValue) {
          userValues = JSON.parse(formbuilderCookieValue)
        }
      } catch (e) {
      // cookie was invalid
      }

      let {userId, userToken, userDigest} = userValues

      // if not make it
      if (!userId) {
        userId = uuidv4()
        userToken = crypto.randomBytes(256).toString('hex')
        userDigest = makeDigest(userId, userToken, serviceToken)
        ...
        formbuilderCookieValue = JSON.stringify(cookieBundle)
        res.cookie('_formbuilder', formbuilderCookieValue)
        req.cookies._formbuilder = formbuilderCookieValue
      } else {
      // check that digest of hash is valid
        const checkHash = makeDigest(userId, userToken, serviceToken)
        if (checkHash !== userDigest) {
          throw new Error(400)
        }
      }
