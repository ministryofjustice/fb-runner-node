...
disallow: /public`
  ...
  app.use((req, res, next) => {
    console.log('url:', req.originalUrl, 'about to get user')
    next()
  })
  ...
  app.use((req, res, next) => {
    console.log('url:', req.originalUrl, 'about to fetch user data')
    next()
  })
  ...
  app.use((req, res, next) => {
    console.log('url:', req.originalUrl, 'about to hit the submitter route')
    next()
  })

  // Convenience route to generate output for current user
  app.use('/api/submitter/:outputType/:outputId', async (req, res) => {
    console.log('HITTING SUBMITTER OUTPUT GENERATION ROUTE')
