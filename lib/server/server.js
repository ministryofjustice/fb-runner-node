  ...
  const generateOutput = async (req, res) => {
    ...
    if (outputType === 'pdf') {
      let pageInstance = {
        ...
        footerTemplate: '<style>.pdfFooter, .pdfFooter * { font-size: 12px; }</style><div class="pdfFooter"><span class="title">Title</span> <span class="pageNumber">pageNumber</span>/<span class="totalPages">totalPages</span></div>'
      }
      ...
      let moutput = await getPageOutput()
      moutput = moutput.replace(/<head>/, `<head><base href="http://localhost:${PORT}">`)
      // const context = await browser.createIncognitoBrowserContext()
      const page = await browser.newPage()
      // await page.goto(`http://localhost:${PORT}/`)

      await page.goto(`data:text/html,${moutput}`, {waitUntil: 'networkidle0'})
      ...
      let email
      if (outputId === 'team') {
        email = 'Please find an application attached'
      } else if (outputId === 'copy') {
        email = 'A copy of your application is attached'
      } else {
        email = `Email body for ${outputId} goes here`
      }
      res.send(email)
    } else {
      throw new Error(404)
