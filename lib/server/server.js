...
disallow: /public`
  ...
  // const fullyQualifiedDomain = 'http://localhost:3000'
  // app.use((req, res, next) => {
  //   if (req.method === 'POST') {
  //     let disallowed
  ...
  //       if (referrer !== fullyQualifiedDomain) {
  ...
  const getTokenSecret = (userId, serviceToken) => {
    // hash this?
  ...
  app.use(async (req, res, next) => {
    console.log('req.servername', req.servername)
    console.log('req.headers', req.headers)
    const userId = req.user.userId || '<UNKNOWN>'
    const secret = getTokenSecret(userId, SERVICE_TOKEN)
    if (req.method === 'POST') {
      const contentType = req.headers['content-type']
      // multipart/form-data; boundary=----WebKitFormBoundarySIuqabhczi9XwjZ5
      // const contentType = req.get('content-type')
      if (!contentType.startsWith('multipart/form-data;')) {
        const {_csrf} = req.body
        const verified = await verifyCsrfToken(secret, _csrf)
        if (!verified) {
          req.csrfRejected = true
        }
      }
    }
    req.csrfToken = await createCsrfToken(secret)
