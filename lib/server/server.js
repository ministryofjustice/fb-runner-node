...
disallow: /public`
  ...
  const generateOutput = async (req, res) => {
    ...
    if (outputType === 'pdf') {
      ...
      const email = `Email body for ${outputId} goes here`
      res.send(email)
    } else {
      throw new Error(404)
    }
  ...
  // API submitter routes
  app.use('/api/submitter/:outputType/:outputId', async (req, res, next) => {
    try {
      let userIdAndToken = req.headers['x-encrypted-user-id-and-token']
      if (!userIdAndToken) {
        // if there's a cookie, let the request pass on and hit this path at the second time of asking with data derived from any user info storied in cookie
      ...
      // console.log('Decrypted userIdAndToken', {userDetails})
      if (!userDetails.userId) {
        // submitterClient.decryptUserIdAndToken
        throw new Error(403)
      }
      const {userId, userToken} = userDetails
      // const data =
      await userDataStoreClient.getData(userId, userToken)
      // load data into req.user so that can then call shared generateOutput middleware function
  ...
  app.use('/api/submitter/:outputType/:outputId', async (req, res) => {
    await generateOutput(req, res)
    // res.send('api submittter output using user data')
  })
