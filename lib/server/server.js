...
disallow: /public`
  ...
  const generateOutput = async (req, res) => {
    ...
    if (outputType === 'pdf') {
      ...
      let email
      if (outputId === 'team') {
        email = getInstanceProperty('service', 'emailTeamTemplate') || 'Please find an application attached'
      } else if (outputId === 'citizen') {
        email = getInstanceProperty('service', 'emailCitizenTemplate') || 'A copy of your application is attached'
      } else {
