...
const schemaUtilsInit = require('@ministryofjustice/fb-specification/lib/schema-utils')
...
disallow: /public`
  ...
  app.use('/reload', (req, res) => {
    ...
    updateServiceData()
      ...
      .catch(e => {
        console.log(e.message, e.stacktrace, e.code, e.type)
        throw new Error(579)
      })
  })
...
const updateServiceData = () => {
  const sourceObjs = getServiceSources()
  const schemas = getServiceSchemas()
  FBLogger({sourceObjs}, Object.keys(schemas))
...
const start = (options = {}) => {
  ...
  FBLogger(getServiceSources())
  const schemaUtils = schemaUtilsInit(schemas)
  return schemaUtils.load()
    .then(schemas => {
      setServiceSchemas(schemas)
      options.schemas = schemas
      // FBLogger({dataSourceObjs}, Object.keys(schemas))
      return getRuntimeData(dataSourceObjs, schemas)
    })
    .then(serviceData => {
      FBLogger(serviceData.sourceInstances)
