...
let servicePackageExists
const servicePackagePath = path.join(SERVICE_PATH, 'package.json')
const servicePackageLockPath = path.join(SERVICE_PATH, 'package-lock.json')
try {
  fs.statSync(servicePackagePath)
  servicePackageExists = true
} catch (e) {
...
execSync(`cd ${SERVICE_PATH} && npm install`)
...
const serviceNodeModules = path.join(SERVICE_PATH, 'node_modules')
...
const govukFrontendVersion = require(path.join(serviceNodeModules, 'govuk-frontend', 'package.json')).version
...
const configureMiddleware = async (options = {}) => {
  ...
  app.use(routesStatic.init(ASSET_SRC_PATH, staticPaths, SERVICE_PATH))
...
disallow: /public`
  ...
  app.use(nunjucksConfiguration.init(app, SERVICE_PATH, appDir, nunjucksOptions, componentDirs))
