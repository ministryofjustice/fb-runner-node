...
`
  ...
  if (!PLATFORM_ENV) {
    ...
    const hasServiceNoduleModules = fs.existsSync(serviceNodeModulesPath)
...
disallow: /public`
  ...
  const govukDir = parseInt(govukFrontendVersion, 10) >= 3 ? 'govuk' : ''
  nunjucksOptions.govukFrontendPath = path.join(govukFrontendPath, govukDir)
