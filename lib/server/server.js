...
const {initControllers} = require('../controller/controller')
...
componentDirs = componentDirs.map(source => {
  ...
  if (sourceEnvVarValue) {
    ...
    console.log('Overriding components module', {sourceEnvVar, sourcePath})
    /* eslint-disable no-console */
  }
  return {
    source,
...
// Add modules
// adding it to componentDirs before loading schemas causes failure
const fbModules = ['savereturn']
fbModules.forEach(module => {
  componentDirs.unshift({
    source: `module:${module}`,
    sourcePath: path.join(kitDir, 'module', module)
  })
})
...
const govukFrontendVersion = require(path.join(govukFrontendPath, 'package.json')).version
