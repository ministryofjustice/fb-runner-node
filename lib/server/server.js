...
disallow: /public`
  ...
  const generateOutput = async (req, res) => {
    ...
    if (outputType === 'pdf') {
      ...
      let email
      if (outputId === 'team') {
        email = getInstanceProperty('service', 'emailTeam') || 'Please find an application attached'
      } else if (outputId === 'copy') {
        email = getInstanceProperty('service', 'emailUser') || 'A copy of your application is attached'
      } else {
        email = 'A copy of the application is attached'
      }
      console.log(`Email output for ${outputId}`, {email})
      res.send(email)
    } else {
      throw new Error(404)
    }
