...
configureMiddlewareStub.callsFake(async () => router)
...
test('When setting up the runner', async t => {
  ...
  const configureMiddlewareStubArgs = configureMiddlewareStub.getCall(0).args

  t.deepEqual(configureSourcesStubArgs, [app, options], 'it should pass the correct args to configureSources')
