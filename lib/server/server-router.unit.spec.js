...
test('When getting the server router', t => {
  const router = getRouter(app, options)

  t.ok(configureMiddlewareStub.calledOnce, 'it should initialise the runner middleware once')
  t.deepEqual(configureMiddlewareStub.getCall(0).args, [app, options], 'it should call configureMiddleware with the expected args')
