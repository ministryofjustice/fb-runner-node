...
const operatorTests = {
  ...
  match: {
    ...
    additionalFailing: {
      'it should return false when trying to match against an undefined value': {
  ...
  nestedVariables: {
    description:
      'When a condition with an identifier that is not a simple key is evaluated',
    additional: {
      ...
      'it should return true for a jsonpath query matching array items that meets the condition, not prefixed with $': {
        identifier: 'array[*]',
        operator: 'is',
        value: 'array item value'
      }
    ...
    additionalFailing: {
      ...
      'it should return false for a jsonpath query matching array items that meets the condition, not prefixed with a $': {
        identifier: 'array[*]',
        operator: 'is',
        value: 'not array item value'
      }
  ...
  same: {
    ...
    additional: {
      'it should return true for a condition asking if all items are same when just specifying the array name': {
        identifier: 'arrayB',
        operator: 'same'
      }
    },
    additionalFailing: {
      'it should return false for a condition asking if all items are same when just specifying the array name': {
        identifier: 'arrayC',
        operator: 'same'
      }
    }
...
const nonePassing = [operatorTests.is.failing, operatorTests.equals.failing]
...
const all = {
  passing: {
    all: cloneDeep(allPassing)
  },
  failing: {
    all: cloneDeep(twoPassing)
  }
}

const any = {
  passing: {
    any: cloneDeep(onePassing)
  },
  failing: {
    any: cloneDeep(nonePassing)
  },
  additional: {
    'it should return true when more than one condition is met': {
      any: cloneDeep(twoPassing)
    },
    'it should return true when all conditions are met': {
      any: cloneDeep(allPassing)
    }
  },
  additionalFailing: {
    'it should return false when less than the number of conditions specified is met': {
      any: cloneDeep(onePassing),
      conditionsMet: 2
    }
  }
...
const exactly = {
  passing: {
    exactly: cloneDeep(onePassing)
  },
  failing: {
    exactly: cloneDeep(nonePassing)
  },
  additional: {
    'it should return true when number of conditions specified is met': {
      exactly: cloneDeep(twoPassing),
      conditionsMet: 2
    }
  },
  additionalFailing: {
    ...
    'it should return false when all conditions are met': {
      exactly: cloneDeep(allPassing)
    },
    'it should return false when less than the number of conditions specified is met': {
      exactly: cloneDeep(onePassing),
      conditionsMet: 2
    },
    'it should return false when more than the number of conditions specified is met': {
      exactly: cloneDeep(allPassing),
      conditionsMet: 2
    }
  }
...
operatorTests.all = all
operatorTests.any = any
operatorTests.exactly = exactly
...
const runTests = (operator, tests) => {
  const sanityCheckTest = test => {
    ...
    if (!tests.description && test.operator !== operator && !test[operator]) {
      ...
      throw new Error(
        `${operator} was not found as either a property or the operator in\n\n${JSON.stringify(
          test,
          null,
          2
        )}`
  ...
  const testDescription =
    tests.description ||
    `When a condition with the "${operator}" ${operatorType} is evaluated`

  test(testDescription, t => {
    if (tests.passing) {
      t.equal(
        evaluate(tests.passing, data),
        true,
        'it should return true when the condition is met'
      )
      const negatedPassing = cloneDeep(tests.passing)

      negatedPassing.negated = true
      t.equal(
        evaluate(negatedPassing, data),
        false,
        'it should return false when the condition is met but the negated property is set to true'
      )
    }
    ...
    if (tests.failing) {
      ...
      t.equal(
        evaluate(tests.failing, data),
        false,
        'it should return false when the condition is not met'
      )

      const negatedFailing = cloneDeep(tests.failing)
      negatedFailing.negated = true
      t.equal(
        evaluate(negatedFailing, data),
        true,
        'it should return true when the condition is not met but the negated property is set to true'
      )
    }
    ...
    if (tests.additionalFailing) {
      Object.keys(tests.additionalFailing).forEach(extraTest => {
        ...
        t.equal(
          evaluate(tests.additionalFailing[extraTest], data),
          false,
          extraTest
        )
      })
