...
const evaluate = (condition, data) => {
  ...
  if (condition.operator) {
    ...
    const identifierType = condition.identifierType || 'input' // TODO: should be derived from schema?
    ...
    if (condition.exactly) {
      const conditionsMet = evaluateConditions(condition.exactly, data)
      console.log(condition)
      if (condition.conditionsMet) {
        console.log(condition.conditionsMet)
      }
      result = conditionsMet.length === (condition.conditionsMet || 1)
    } else if (condition.any) {
      const conditionsMet = evaluateConditions(condition.any, data)
      result = conditionsMet.length >= (condition.conditionsMet || 1)
    } else if (condition.all) {
      const conditionsMet = evaluateConditions(condition.all, data)
