...
const {
  ...
  expandInstance,
  getInstanceTitle,
  getInstanceSummaryTitle
} = serviceData

test('When getInstanceTitle is invoked', t => {
  const instanceLabel = 'someLabel'

  function setTextOnInstance (texts) {
    setServiceInstances(deepClone({
      [instanceLabel]: {
        ...texts
      }
    }))
  ...
  t.equal(getInstanceTitle(instanceLabel), 'some heading', 'shouldn\'t use any summary text without specifying the isSummary signature')
  t.equal(getInstanceTitle(instanceLabel, true), 'summary text', 'should use the summary text when available')

  setTextOnInstance({
    label: 'some label',
    heading: 'some heading'
  })
  t.equal(getInstanceTitle(instanceLabel), 'some heading', 'should prefer headings over labels')

  setTextOnInstance({})
  t.equal(getInstanceTitle(instanceLabel), instanceLabel, 'should return the ID if no text could be found')

  setTextOnInstance({
    title: 'some title',
    text: 'some text'
  })
  t.equal(getInstanceTitle(instanceLabel), 'some title', 'should prefer the title over the text property')

  t.end()
})

test('When getInstanceSummaryTitle is invoked', t => {
  const instanceLabel = 'someLabel'
  ...
  setTextOnInstance({summaryText: 'summary text'})
  t.equal(getInstanceSummaryTitle(instanceLabel), 'summary text', 'should use the summary text when available')

  setTextOnInstance({
    summaryLabel: 'summary label',
    summaryText: 'summary text'
  })
  t.equal(getInstanceSummaryTitle(instanceLabel), 'summary label', 'should use the summary label over the summary text')

  setTextOnInstance({
    summaryLabel: 'summary label',
    summaryLegend: 'summary legend',
    summaryText: 'summary text'
  })
  t.equal(getInstanceSummaryTitle(instanceLabel), 'summary legend', 'should use the summary legend over the label and text')

  setTextOnInstance({
    title: 'some title'
  })
  t.equal(getInstanceSummaryTitle(instanceLabel), 'some title', 'should resort to a title when no summary text is provided')
...
test('When userData is required ', t => {
  ...
  t.equal(typeof getInstanceSummaryTitle, 'function', 'it should export the getInstanceSummaryTitle method')
  t.end()
})
...
test('When writing instance file to writable location', async t => {
  t.plan(2)
