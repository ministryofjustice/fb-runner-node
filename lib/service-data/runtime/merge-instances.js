...
const log = debug('runner:merge-instances')

debug.enable('runner:merge-instances')
 ...
 **/
function annotateSourceInstances ({ source, instances, ...sources }) {
  return {
    ...sources,
    source,
      ...
      Object
        ...
        .forEach((instance) => {
          ...
          if (Reflect.has(instances, _id)) {
            log(`Already have "${_id}" -- continuing`)
          } else {
...
function merge (sources) {
  return flattenSources(
    sources.map((source) => annotateSourceInstances(source))
  )
}
