...
function flattenSources (sources) {
  ...
  const instancesBySourceName = {}

  sources
    .forEach(({source: sourceName, instances}) => {
      instancesBySourceName[sourceName] = instances.reduce((accumulator, instance) => {
        return {
          ...accumulator,
  ...
  function expandIsa (instance) {
    ...
    const isaInstance = expandInstanceRef(instancesBySourceName[isaSource][isaId])
  ...
  sourceNames
    .forEach((sourceName) => {
      Object
        .values(instancesBySourceName[sourceName])
        .forEach((instance) => {
          const {_id} = instance
