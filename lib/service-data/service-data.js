...
const log = debug('runner:service-data:log')
const error = debug('runner:service-data:error')
...
external.loadServiceData = async function loadServiceData () {
  ...
  if (schemas['definition.namespace']) {
    log('Updating models to namespace ...')
    ...
    try {
      ...
      error('Updating models to namespace failed')
    }
  } else if (schemas['definition.model']) {
    log('Not updating models to namespace. Exiting')
...
external.getSchemaNameByCategory = (category) => external.getSchemaCategories()[category] || []
...
external.getInstance = (id, defaultValue) => {
  ...
  if (!instance) {
    instance = jsonPath.query(external.getServiceInstances(), `$..[?(@._id === "${id}")]`)[0] || defaultValue // ignore: node_sqli_injection
  }
...
external.getDiscreteInstance = (id) => {
  const topLevelInstance = external.getServiceInstances()[id] // ignore: node_sqli_injection
  if (topLevelInstance) {
  ...
  for (const paths of allPaths) {
    ...
    const potentialInstance = external.getInstance(toplevelId) //ignore: node_sqli_injection
    if (!potentialInstance) {
      break
    }
    const potentialSourceInstance = external.getSourceInstance(potentialInstance._id, potentialInstance.$source) // ignore: node_sqli_injection
    if (!potentialSourceInstance) {
      break
    }
    const [containsId] = jsonPath.query(potentialSourceInstance, `$..[?(@._id === "${id}")]`) // ignore: node_sqli_injection
    if (containsId) {
...
external.getInstancesByProperty = (property) => {
  const instances = external.getServiceInstances() // ignore: node_sqli_injection
...
external.deleteInstance = (_id) => {
  const discreteInstance = external.getDiscreteInstance(_id) // ignore: node_sqli_injection
...
external.setInstance = (instance) => {
  ...
  const discreteInstance = external.getDiscreteInstance(id) // ignore: node_sqli_injection
  if (!discreteInstance) {
    return throwRejectedPromise({
...
external.setInstanceProperty = (id, property, value) => {
  ...
  const instance = external.getDiscreteInstance(id) // ignore: node_sqli_injection
  let newInstance = external.getSourceInstance(id, instance.$source) // ignore: node_sqli_injection
  ...
  if (!newInstance._type.includes('string') && (property === 'name' || property === 'value')) {
    const updatePropertyReferences = (propName, query, applyFn) => {
      const allInstances = external.getServiceInstances() // ignore: node_sqli_injection
      const updateIdentifier = (instId) => {
        let inst = instId
        if (typeof inst === 'string') {
          inst = allInstances[instId]
          inst = external.getSourceInstance(instId, inst.$source) // ignore: node_sqli_injection
        }
...
external.createInstance = (instance, options = {}) => {
  ...
  if (addId) {
    ...
    const addInstance = external.getDiscreteInstance(addId) // ignore: node_sqli_injection
    if (!addInstance) {
      return throwRejectedPromise({
...
external.getSourceInstance = (id, source = 'service', defaultValue) => {
  const sourceData = external.getSourceInstances()[source] // ignore: node_sqli_injection
  if (!sourceData) {
    return
  }
  ...
  return jsonPath.query(sourceData, `$..[?(@._id === "${id}")]`)[0] || defaultValue // ignore: node_sqli_injection
}
...
external.getSourceInstanceProperty = (id, property, source = 'service', defaultValue) => {
  const instance = external.getSourceInstance(id, source) || {} // ignore: node_sqli_injection
  return get(instance, property, defaultValue)
}
...
external.expandInstance = (instance, addError) => {
  ...
  if (instance._isa) {
    ...
    if (!service) {
      const isaDiscreteInstance = external.getDiscreteInstance(_isa) // ignore: node_sqli_injection
    ...
    let isaSourceInstance = external.getSourceInstance(_isa, service) // ignore: node_sqli_injection
