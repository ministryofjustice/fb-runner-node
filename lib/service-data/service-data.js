...
external.getSchemaPropertyAllowableTypes = (schemaName, property) => {
  ...
  // }
  let allowables = []
  let accepts = get(schema, `properties["${property}"].accepts`)
  ...
  if (accepts) {
    allowables = allowables.filter(allowableName => {
      let allowableSchema = external.getServiceSchema(allowableName)
      let allowableCategory = allowableSchema.category || []
...
external.getString = (_id, lang) => {
  // TODO: pass through language details
  return external.getInstanceProperty(_id, `value:${lang}`) || external.getInstanceProperty(_id, 'value')
}
...
external.getPageInstances = () => {
  ...
  const pages = Object.keys(serviceInstances).map(_id => serviceInstances[_id])
    .filter(instance => instance._type && instance._type.startsWith('page.'))
  return pages
}
  ...
  if (property === 'repeatable' && value) {
    if (!newInstance.model) {
      if (newInstance._type.match(/^(page\.|fieldset|group)/)) {
        let modelSeed = 'model_auto__1'
        let modelSeedCheck = true
        while (modelSeedCheck) {
          if (external.getInstanceIdByPropertyValue('model', modelSeed)) {
            modelSeed = modelSeed.replace(/_(\d+)$/, (m, m1) => {
              const newSuffix = m1 * 1 + 1
              return `_${newSuffix}`
            })
          } else {
            modelSeedCheck = false
          }
        }
        set(newInstance, 'model', modelSeed)
      }
    }
  }
