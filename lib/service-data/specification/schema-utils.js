...
function schemaUtils (specs = [{}]) {
  ...
  specs.forEach(({$idRoot, path}) => {
    ...
    specsIdRootToPathMap[$idRoot] = path
    specsNamePrefix[name] = $idRoot
    specsIdRootToNameMap[$idRoot] = name // default is zero-length string
  })
  ...
  const specsNameMatchOrder = Object.keys(specsNamePrefix)
    .sort()
    .reverse()
  ...
  function mapIdToDir ($id) {
    ...
    return specsIdRootToPathMap[$idRoot]
  }
  ...
  async function load () {
    const schemas = {}
    await Promise.all(specs.map((spec) => loadSchemaFromPath(schemas, spec.path)))
    return schemas
  }
