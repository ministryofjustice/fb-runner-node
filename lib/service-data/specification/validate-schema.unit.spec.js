...
const specsAPath = path.resolve('./data/service-data/specification/specs-a')
...
const getExpected = (file) => require(path.join(path.resolve('./data/service-data/specification/expected'), `${file}.json`))

function updateFixedPaths (results) {
  return results.reduce((accumulator, individualResult) => {
    individualResult.path = individualResult.path.replace(/.*\/data\/service-data\/specification\/specs-a/, '/data/specs-a')

    return accumulator.concat(individualResult)
  }, [])
}
...
test('When validating files that are expected to be valid against a schema but which are not', t => {
  ...
  validateSchema('definition.data', {specs, valid: invalidFiles})
    .then(result => {
      ...
      const dataSrc = 'data/service-data/specification/specs-a/specifications/definition/data/data/invalid/'
      const correctPaths =
        valid[0].path.includes(`${dataSrc}definition.data.invalid._id.json`) &&
...
test('When validating files that are expected to be invalid against a schema but which are not', t => {
  ...
  validateSchema('definition.data', {specs, invalid: validFiles})
    .then(result => {
      ...
      const dataSrc = 'data/service-data/specification/specs-a/specifications/definition/data/data/valid/'
      const correctPaths =
        invalid[0].path.includes(`${dataSrc}definition.data.json`) &&
