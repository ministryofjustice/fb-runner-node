...
const callRoutesStatic = ({assetsUrlPrefix = '', paths = [], componentDirs}) => {
  const app = express()
  app.use(routesStatic.init({assetsUrlPrefix, staticPaths: paths, componentDirs}))
  return app
}
...
test('When a file from an assets folder is requested', async t => {
  ...
  const app = callRoutesStatic({componentDirs})
