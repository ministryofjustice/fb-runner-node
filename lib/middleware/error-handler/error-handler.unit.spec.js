...
test('When errorHandler is required ', t => {
  t.equal(typeof errorHandler, 'function', 'it should export the locals method')
...
test('When an unauthorised route is hit', t => {
  const {req, assertStatusCode} = invokeError(errorHandler().handle, t, new Error(401))

  assertStatusCode(401)
  t.equals(req.disqualified, true, 'should set req.disqualified to true')
  t.end()
})
