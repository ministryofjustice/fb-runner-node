...
const errorHandler = (options = {}) => {
  ...
  external.handle = (err, req, res, next) => {
    if (err) {
      const errMessage = `Error: ${req.originalUrl} - name: ${err.name} - message: ${err.message} - code: ${err.code}`
      if (req.log) {
        req.log.error({err, req}, errMessage)
      } else {
        FBLogger(errMessage, err.stack, err.stacktrace)
      }
      ...
      external.render(req, res, errCode, err.renderError)
    }
  }
