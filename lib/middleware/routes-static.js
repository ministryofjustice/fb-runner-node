...
const routesStatic = (assetsUrlPrefix, staticPaths = []) => {
  ...
  router.use(assetsUrlPrefix, express.static(path.resolve('assets')))
  router.use(assetsUrlPrefix, express.static(path.resolve('node_modules', '@govuk-frontend', 'frontend', 'assets')))
