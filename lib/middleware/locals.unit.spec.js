...
const getLocalInstance = (options = {}) => {
  const localsInstance = locals('ENV', 'assetPath', 'env')
  const next = () => {}
  const req = {
    headers: {
      host: 'domain',
...
test('When locals middleware is called', t => {
  ...
  t.equals(req.servername, 'http://domain', 'it should set req.ENV')
...
test('When locals middleware is called and x-forwarded-proto header is set', t => {
  ...
  t.equals(req.servername, 'https://domain', 'it should set req.ENV')
