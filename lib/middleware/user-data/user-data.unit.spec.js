...
const {loadUserData, saveUserData, getUserDataMethods} = userDataMethods
...
test('When getting user data', t => {
  ...
  const next = () => {
    ...
    t.deepEqual(getAllData(), {input: {a: {b: true}}, count: {}}, 'it should add getUserData method')
  }
  loadUserData(req, res, next)
})
...
test('When getting count values', t => {
  ...
  t.equal(userData.getUserCountProperty('notthere'), 1, 'it should return defaulted value for getUserCountProperty when no value exists')
