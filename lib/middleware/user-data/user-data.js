...
external.getUserDataMethods = (data, params, additional = {}) => {
  ...
  const getDataScope = (dataScope) => {
    let acutalScope = dataScope || scope
  ...
  const getScopedUserData = (scopeArgs = {}) => {
    const data = deepClone(getUserData())
    data._scopes = getAllData() || {}
  ...
  const logger = additional.req && additional.req.log ? additional.req.log : () => {}
...
external.loadUserData = async (req, res) => {
  ...
  if (!req.newSession) {
    try {
      data = await loadData({userId, userToken}, req.log, req.session)
    } catch (err) {
      if (err.code === 404) {
