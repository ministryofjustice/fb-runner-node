...
external.init = (options = {}) => {
  ...
  router.use((req, res, next) => {
    ...
    if (req.originalUrl.includes('api')) {
      console.log('DEBUG - user-session - x-header', req.originalUrl, {userIdAndToken})
      console.log('DEBUG - user-session - all headers', req.originalUrl, JSON.stringify(req.headers, null, 2))
    }
    if (userIdAndToken) {
      if (req.method !== 'GET') {
