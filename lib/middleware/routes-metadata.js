...
const metadataRouter = (siteData) => {
  jsonLoader.load(siteData).then(loadedInstances => {
    const testJourney = mergeInstances.merge(loadedInstances)
    Object.keys(testJourney).forEach(key => {
      const page = testJourney[key]
      urls[page.url] = page
      pages[page._id] = page
    })
