...
const metadataRouter = (serviceData, schemas) => {
  ...
  router.use((req, res, next) => {
    ...
    if (!handlerData) {
      ...
      skipComponents(pageInstance, getAll())
      ...
      if (POST) {
        ...
        validateInput(pageInstance, getPath, evaluateInput)
      ...
      // Update name values
      updateControlNames(pageInstance, getPath)

      // Format all the properties which need to be
      formatProperties(pageInstance, getAll())
