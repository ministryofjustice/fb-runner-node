...
const pageHandler = async (req, res, next) => {
  ...
  // const languages = getInstanceProperty('service', 'languages') || []
  ...
  if (!handlerData) {
    ...
    if (POST) {
      ...
      if (add) { // } && pageInstance.$validated) {
        ...
        if (repeatableId) {
          ...
          if (repeatablePage) {
            url = userData.pagesMethods.getUrl(repeatablePage._id, repeatablePage.params, userData.contentLang)
      ...
      if (!EDITMODE) {
        if (pageInstance.$validated) {
          if (CHANGEPAGE) {
            ...
            let nextUrl = pagesMethods.getUrl(nextPage._id, nextPage.params, userData.contentLang)
    ...
    if (req.newSession) {
      pageInstance = produce(pageInstance, draft => {
        draft.cookieMessage = getInstanceLangProperty('cookies.message', 'value', pageInstance.contentLang)
        return draft
      })
    }
    ...
    renderPage(res, pageInstance, {GA_TRACKING_ID, req})
  }
}

const renderPage = async (res, pageInstance, context) => {
  try {
    let output = await nunjucksConfiguration.renderPage(pageInstance, context)
    res.send(output)
  } catch (e) {
    res.sendStatus(404)
  }
