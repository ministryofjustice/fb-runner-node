...
const pageHandler = async (req, res, next) => {
  ...
  if (!handlerData) {
    ...
    if (pageController.preUpdate) {
      try {
        pageInstance = await pageController.preUpdate(pageInstance, userData)
      } catch (e) {
        return next(e)
      }
      // await userData.saveData() // is this necessary?
    ...
    if (modules.savereturn && modules.savereturn.preRender) {
      pageInstance = await modules.savereturn.preRender(pageInstance, userData)
    }

    // fbModules.forEach(mod => {
    //   if (mod.preRender) {
    //     pageInstance = await mod.preRender(pageInstance, userData)
    //   }
    // })
