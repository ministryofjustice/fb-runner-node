...
const pageHandler = async (req, res) => {
  ...
  if (!handlerData) {
...

    {
      ...
      if (components.some(({_type}) => 'fileupload')) {
      ...
      if (components.some(({_type}) => 'mojFileupload')) {
        // Handle MOJ multi-part forms and uploads
        pageInstance = await setMOJMultipartForm(pageInstance, userData)
        if (pageInstance.encType) {
          if (req.method === 'POST') {
            pageInstance = await processMOJUploads(pageInstance, userData)
          }
