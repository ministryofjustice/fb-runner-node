...
const setMOJUploads = require('~/fb-runner-node/page/set-moj-uploads/set-moj-uploads')
const setMOJUploadControlsMaxSize = require('~/fb-runner-node/page/set-moj-uploads/set-moj-upload-controls-max-size')
...
const pageHandler = async (req, res) => {
  ...
  if (!handlerData) {
...

    {
      ...
      const hasMUpload = components.some(({_type}) => 'mojUpload')
      ...
      if (hasFUpload || hasMUpload) {
        ...
        if (pageInstance.encType) {
          ...
          if (hasMUpload) {
            setMOJUploadControlsMaxSize(pageInstance)
          }
          ...
          if (hasMUpload) {
            pageInstance = await setMOJUploads(pageInstance, userData)
          }
        }
      }
    }
