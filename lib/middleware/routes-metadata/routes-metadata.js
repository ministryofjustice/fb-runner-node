...
async function executePreFlight ({preFlight}, pageInstance, userData, ...args) {
  if (preFlight) {
    pageInstance = await preFlight(pageInstance, userData, ...args)
  }
...
async function pageHandler (req, res) {
  ...
  if (!handlerData) {
    ...
    pageInstance = await executeSetContents(pageController, pageInstance, userData, POST)
    if (hasRedirect(pageInstance)) return handleRedirect(res, pageInstance, userData)
    ...
    if (POST) {
      ...
      if (isUploadCheckPage(pageInstance)) {
        if (POST) {
          return handleUploadCheckPage(res, pageController, pageInstance, userData)
        }
      }

      if (isUploadSummaryPage(pageInstance)) {
        if (POST) {
          return handleUploadSummaryPage(res, pageController, pageInstance, userData)
        }
      }
