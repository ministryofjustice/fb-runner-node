...
const {getInstanceController} = require('../../page/controller/controller')
...
const pageHandler = async (req, res, next) => {
  ...
  if (!handlerData) {
    ...
    if (POST) {
      ...
      const {addFile, removeFile, removeSlot} = userData.getBodyInput()
      const skipValidation = addFile || removeFile || removeSlot
      if (!skipValidation) {
