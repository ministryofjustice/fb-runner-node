...
async function handleFileUploadComponents (pageInstance, userData) {
  setFileUploadControlsMaxSize(pageInstance)

  return setFileUploads(pageInstance, userData)
}

async function handleUploadComponents (pageInstance, userData) {
  setUploadControlsMaxSize(pageInstance)

  return setUploads(pageInstance, userData)
}
...

    {
      ...
      if (hasFileUploads || hasUploads) {
        ...
        if (pageInstance.encType) {
          ...
          if (hasFileUploads) {
            pageInstance = await handleFileUploadComponents(pageInstance, userData)
          }

          if (hasUploads) {
            pageInstance = await handleUploadComponents(pageInstance, userData)
          }
        }
      }
    }
