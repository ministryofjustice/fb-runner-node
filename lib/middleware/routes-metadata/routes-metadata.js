...
const pageHandler = async (req, res, next) => {
  ...
  if (!handlerData) {
    ...
    if (pageInstance._id === 'page.start') {
      let serviceUrl = getServiceUrl(userData.contentLang)
      if (serviceUrl) {
        const referrer = req.headers['referer']
        if (referrer !== serviceUrl) {
    ...
    if (!EDITMODE) {
      ...
      if (pageInstance.redirect) {
        ...
        if (previousPage) {
          const visitedPath = 'visited_form_pages'
          const originalScope = userData.getScope()
          userData.setScope('visited')

          let visitedPages = userData.getUserDataProperty(visitedPath, [], 'visited')

          visitedPages = visitedPages.filter(({id}) => id !== pageInstance._id)

          visitedPages.push({
            id: pageInstance._id
          })

          userData.setUserDataProperty(visitedPath, visitedPages, 'visited')
          userData.setUserDataProperty('latestURL', currentUrl, 'visited')

          await userData.saveData()
          userData.setScope(originalScope)
        }
      }
    }
    ...
    renderPage(res, pageInstance, context, next)
  }
}

const renderPage = async (res, pageInstance, context, next) => {
  try {
    ...
    if (PLATFORM_ENV) {
      ...
      return nunjucksConfiguration.debugError(err, next)
    }
  }
}
