...
external.getFormattedError = (position, controlInstance, errorType, error, lang) => {
  ...
  // look up instance error string for specific position (summary or inline) or 'any' position
  let errorString = getInstanceErrorString(instanceErrorStrings, position, lang) || getInstanceErrorString(instanceErrorStrings, 'any', lang)
...
external.setSummaryError = (pageInstance, controlInstance, errorType, error) => {
  ...
  // if (errorType.includes('maxSize')) console.log('setSummaryError', JSON.stringify(pageInstance, null, 2))
