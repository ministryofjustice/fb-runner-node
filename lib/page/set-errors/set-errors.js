...
external.adjustErrorObjs = (pageInstance, errorObjs) => {
  ...
  console.log('adjustErrorObjs', {errorObjs})
...
const getErrorArgs = (controlInstance = {}, error, lang) => {
  console.log('getErrorArgs', {controlInstance, error})
...
* @private
*/
external.getFormattedError = (location, controlInstance, errorType, error, lang) => {
  ...
  if (lang) {
    const langErrorString = instanceErrorStrings[`${location}:${lang}`]
    if (langErrorString) {
      errorString = langErrorString
    }
  }
  ...
  if (errorString === undefined) {
    ...
    errorString = external.getString(`error.${errorType}.${controlInstance._type}.${location}`, lang)
    if (errorString === undefined) {
      // errorType.controlType
      // eg. error.required.text
      errorString = external.getString(`error.${errorType}.${controlInstance._type}`, lang)
      if (errorString === undefined) {
        // errorType.location
        // eg. error.required.inline
        errorString = external.getString(`error.${errorType}.${location}`, lang)
        if (errorString === undefined) {
          // errorType
          // eg. error.required
          errorString = external.getString(`error.${errorType}`, lang)
          if (errorString === undefined) {
...
external.setControlError = (pageInstance, controlInstance, errorType, error) => {
  console.log('setControlError', {controlInstance, errorType, error})
...
external.setSummaryError = (pageInstance, controlInstance, errorType, error) => {
  ...
  if (controlInstance) {
    ...
    console.log({_id, errorType}, controlInstance.compositeName, controlInstance.name)
  ...
  // console.log({controlInstance}, controlInstance.errors, controlInstance.items)
  // console.log({error})
  const item1id = controlInstance.items ? controlInstance.items[0].compositeName : undefined
  // let errorId = item1id || controlInstance.compositeName || controlInstance.name || controlInstance.id || controlInstance._id || ''
  ...
  errorId = errorId.replace(/COMPOSITE_/, 'COMPOSITE--')
