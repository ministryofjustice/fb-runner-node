...
external.setErrors = (pageInstance, errorObjs) => {
  const erroredPageInstance = produce(pageInstance, draft => {
    external.setControlErrors(errorObjs)
    external.setSummaryErrors(draft, errorObjs)
    return draft
  })
...
external.setControlErrors = (errorObjs = []) => {
  errorObjs.forEach(errorObj => {
    external.setControlError(errorObj.instance, errorObj.errorType, errorObj.error)
  })
}

external.setControlError = (controlInstance, errorType, error) => {
