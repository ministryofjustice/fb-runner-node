...
function getInstanceErrorString (errorStrings = {}, position, lang) {
  /**
   *  If we have `lang` then return `position:lang` or default to `position`
   *  otherwise, return `position`
   */
  return lang
    ? errorStrings[`${position}:${lang}`] || errorStrings[position]
    : errorStrings[position]
}
...
external.getFormattedError = (position, controlInstance, errorType, error, lang) => {
  ...
  // look up the instance error string for specific position (summary or inline)
  let errorString = getInstanceErrorString(instanceErrorStrings, position, lang)

  // is it STILL undefined?
  if (errorString === undefined) {
    // look up the instance error string (empty strings are fine and will not reach here)
    errorString = getInstanceErrorString(instanceErrorStrings, 'any', lang)

    // if no error string defined on the control, look up error strings
    const errorLookups = [
  ...
  // is it STILL undefined?
  if (errorString === undefined) {
    return errorType // Give up. Return the errorType
  } else {
    const errorArgs = getErrorArgs(controlInstance, error, lang)
    errorString = format(String(errorString), errorArgs, {lang}) // slightly safer than `toString()`
    // TODO: Move this to format
    return errorString.replace(/<\/p>\s*<p>/g, '<br>').replace(/<\/*p>/g, '')
  }
}
