...
test('When formatting error messages', t => {
  ...
  serviceData.setServiceInstances({
    'errors.azimuth': {boom: 'It went boom'}
  })
  // sinon.stub(kaboodle, 'getInstance').callsFake(() => {
  //   return {boom: 'It went boom'}
  // })
...
test('When setting a control error', t => {
  ...
  setControlError(controlInstance, errorType)
  t.equal(controlInstance.error, 'boom', 'it should set the instance’s error property')
  t.end()
})
...
test('When setting control errors', t => {
  ...
  const argsCheck = setControlErrorSpy.getCall(0).calledWith(errors[0].instance, errors[0].errorType, undefined) && setControlErrorSpy.getCall(1).calledWith(errors[1].instance, errors[1].errorType, errors[1].error)
...
test('When setting summary heading', t => {
  serviceData.setServiceInstances({
    'errors.summary': {
      heading: 'There {errorCount, plural, 0{were no problems} 1{was one problem} other{were {errorCount} problems}}'
    }
  })
...
test('When setting all errors', t => {
  ...
  t.ok(setControlErrorsStub.calledOnceWithExactly(errorObjs), 'it should call setControlErrors with the correct args')
