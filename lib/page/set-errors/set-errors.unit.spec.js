...
test('When setting a summary error for a field with items and name', t => {
  const {pageInstance, controlInstance, errorType} = getInstanceParts()
  controlInstance.items = []
  setSummaryError(pageInstance, controlInstance, errorType)
  t.deepEqual(pageInstance.errorList, [{
    html: 'boom',
    href: '#controlname-error'
  }], 'it should add the anchor for the error element based on the instance name')
  t.end()
})

test('When setting a summary error for a field with items with no name', t => {
  const {pageInstance, controlInstance, errorType} = getInstanceParts()
  delete controlInstance.name
  controlInstance.items = []
  setSummaryError(pageInstance, controlInstance, errorType)
  t.deepEqual(pageInstance.errorList, [{
    html: 'boom',
    href: '#control-error'
  }], 'it should add the anchor for the error element based on the instance _id')
  t.end()
})
...
test('When setting summary errors', t => {
  ...
  ], 'it should set the page summary error heading')
