...
const userData = (input) => {
  return {
    getScopedUserData: (scopedArgs = {}) => {
      const param = Object.assign({
        x: 'param_value'
      }, scopedArgs.param)
      return Object.assign({}, input, {_scopes: {
        input,
        param
      }})
    }
...
test('When updating a page', t => {
  ...
  t.deepEqual(formatProperties({nocontent: '{x}'}, userData({x: 'value'})), {nocontent: '{x}'}, 'it should leave non-content properties intact')

  t.deepEqual(formatProperties({unprocessed: '{x}'}, userData({x: 'value'})), {unprocessed: '{x}'}, 'it should leave properties marked as surplus intact')

  t.deepEqual(formatProperties({components: [{
    _type: 'foo',
    heading: '{x}'
  }, {
    _type: 'bar',
  ...
  contentProps.forEach(prop => {
    ...
    const pageInstanceWithParam = {[prop]: '[#x]'}
    t.deepEqual(formatProperties(pageInstanceWithParam, userData()), {[prop]: 'param_value'}, `it should update ${prop} properties containing param syntax`)
  })
  ...
  const pageInstanceWithHtmlAndParam = {label: {html: '[#x]'}}
  t.deepEqual(formatProperties(pageInstanceWithHtmlAndParam, userData()), {label: {html: 'param_value'}}, 'it should update nested html properties containing param')

  const pageInstanceWithParam = {heading: '[#z]'}
  t.deepEqual(formatProperties(pageInstanceWithParam, userData(), {z: 'passed_param_value'}), {heading: 'passed_param_value'}, 'it should update properties containing param syntax when params are passed explicitly')

  const pageInstanceWithDefaultValue = {
    _type: 'defaultTypeSchema',
    defaultTest: undefined
  }
  t.deepEqual(formatProperties(pageInstanceWithDefaultValue, userData()).defaultTest, 'defaultTest default value', 'it should use the default value when no value specified')

  const pageInstanceWithSpecifiedValue = {
    _type: 'defaultTypeSchema',
    defaultTest: 'specified value'
  }
  t.deepEqual(formatProperties(pageInstanceWithSpecifiedValue, userData()).defaultTest, pageInstanceWithSpecifiedValue.defaultTest, 'it should not overwrite specified values with default ones')
...
test('When updating a page and a lang is specified', t => {
  ...
  t.deepEqual(formatProperties(pageInstance, userData(), {}), pageInstance, 'it should leave properties untouched if no lang specified')
...
test('When updating a page in edit mode', t => {
  ...
  const data = userData()
  t.deepEqual(formatProperties(emptyPageInstance, data, {}), emptyPageInstance, 'it should leave properties untouched if no edit mode passed')
