    ...
    .forEach(control => {
      ...
      if (!control.hint && acceptHints) {
        acceptHints.forEach((type, index, arr) => {
          ...
          let hintValue = getString(`upload.hint.type.${typeLookup}`, userData.contentLang)
          if (!hintValue) {
            ...
            hintValue = getString(`upload.hint.type.${typeLookup}`, userData.contentLang) || typeLookup
          }
          arr[index] = hintValue
        })
      ...
      if (maxAvailableSlots) {
        ...
        if (slots < maxAvailableSlots) {
          control.addFile = getString('upload.file.button.add', userData.contentLang)
        }
      }

      if (!control.hint) {
        let controlHintString = getString('upload.hint', userData.contentLang)
      ...
      if (slots) {
        let uploadExplanation = getString('upload.hint.timing', userData.contentLang)
      ...
      const fileDescription = getString('upload.file.description', userData.contentLang)
      const fileRemoveText = getString('upload.file.button.remove', userData.contentLang)
      const slotLabel = getString('upload.slot.label', userData.contentLang)
      const removeSlotText = getString('upload.slot.button.remove', userData.contentLang)
      ...
      const summaryList = currentFiles.map((file, index) => {
        ...
        const filetype = getString(`upload.file.type.${mimetypeKey}`) || file.mimetype.replace(/.*\//, '').replace(/\+.*/g, '')
      ...
      if (summaryList.length) {
        const heading = getString('upload.files.heading', userData.contentLang)
