...
const getUploadInstances = async (pageInstance, userData) => {
  ...
  const {components: [control]} = pageInstance
  const fileUploads = control.fileUploads
  return {
    control,
    fileUploads
  }
...
test('Setting up "single" type uploads and no files have been uploaded', async t => {
  const {userData} = setupUserDataStubs()

  const pageInstance = getPageInstanceWithUploadComponent({})

  const {control, fileUploads, fileUploads: [fileUpload]} = await getUploadInstances(pageInstance, userData)
  ...
  t.equal(fileUploads.length, 1, 'it should prepare 1 upload slot')
  t.equal(control.slots, 1, 'it should set the number of slots the control needs to 1')
...
test('Setting up a single upload', async t => {
  ...
  const {fileUploads: [fileUpload]} = await getUploadInstances(pageInstance, userData)
