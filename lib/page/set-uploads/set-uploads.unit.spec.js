...
test('When a user asks for an additional upload slot and no slots already exist', async t => {
  ...
  t.equal(uploadSlots.length, 2, 'it should increase the number of upload slots to 1')
...
test('When setting up a file upload which has a file already uploaded', async t => {
  ...
  t.equal(uploadKey.html, 'foo.png (png, 10KB)', 'it should set the text describing the uploaded file')
  ...
  t.equal(uploadRemove.classes, 'fb-action-secondary fb-action--delete', 'it should set any classes required for the remove button text for the uploaded file')
