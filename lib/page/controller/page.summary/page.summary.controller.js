...
SummaryController.setContents = (pageInstance, userData, res, skipActions) => {
  ...
  while (nextPage) {
    ...
    if (!stepInstance.$skipPage) {
      ...
      const sectionHeading = stepInstance.summarySectionHeading || stepInstance.sectionHeading
      const previousSectionHeading = previousInstance.summarySectionHeading || previousInstance.sectionHeading
      ...
      let pageHeading = stepInstance.summaryHeading || stepInstance.heading
      if (names.length) {
        if (!pageHeading) {
      ...
      names.forEach(nameInstance => {
        ...
        let question = nameInstance.summaryLabel || nameInstance.summaryLegend || nameInstance.legend || nameInstance.label || getInstanceSummaryTitle(_id)
        ...
        if (nameInstance._type === 'checkboxes') {
          if (nameInstance.items) {
            const values = nameInstance.items.filter(item => {
              ...
              return getInstanceSummaryTitle(item._id) || item.value
            })
            if (values.length === 0) {
              values.push('None')
            }
          ...
          if (nameInstance.items) {
            ...
            answer = nameItem ? getInstanceSummaryTitle(nameItem._id) : answer
          }
        }
        if (_type === 'fileupload') {
