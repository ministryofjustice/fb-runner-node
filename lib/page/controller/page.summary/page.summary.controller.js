...
SummaryController.setContents = (pageInstance, userData, res, skipActions) => {
  ...
  while (nextPage) {
    ...
    if (!stepInstance.$skipPage) {
      ...
      const sectionHeading = stepInstance.sectionHeadingSummary || stepInstance.sectionHeading
      const previousSectionHeading = previousInstance.sectionHeadingSummary || previousInstance.sectionHeading
      ...
      let pageHeading = stepInstance.headingSummary || stepInstance.heading
      if (names.length) {
        if (!pageHeading) {
      ...
      names.forEach(nameInstance => {
        ...
        let question = nameInstance.labelSummary || nameInstance.legendSummary || nameInstance.legend || nameInstance.label || getInstanceTitleSummary(_id)
