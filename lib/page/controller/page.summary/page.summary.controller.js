...
SummaryController.postValidation = async (pageInstance, userData) => {
  ...
  if (submitterClient) {
    ...
    const defaultSubmission = {
      ...
      to,
      from,
      subject: 'Form Builder submission subject',
      body_parts: {
    ...
    const submissions = [defaultSubmission]
    ...
    if (defaultCCAddress) {
      ...
      if (ccAddress) {
        const copySubmission = deepClone(defaultSubmission)
        copySubmission.to = ccAddress
        copySubmission['body_parts']['text/plain'] = `${emailUrl}copy`
        submissions.push(copySubmission)
      }
    }
    try {
      // const status =
