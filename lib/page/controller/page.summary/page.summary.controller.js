...
const {SERVICETEAM_EMAIL} = CONSTANTS
...
SummaryController.postValidation = async (pageInstance, userData) => {
  ...
  if (submitterClient) {
    ...
    // if (teamEmail) {
    //   to = teamEmail ? userData.getUserDataProperty(teamEmail) : SERVICETEAM_EMAIL
    // }
    to = SERVICETEAM_EMAIL
    try {
      to = formatEmail(to)
    } catch (e) {
      //
    }
    let toParts = to.split(/,\s*/)
    toParts = toParts.map(address => {
      address = address.trim()
      if (!address.startsWith('e:')) {
        return address
      }
      const addressLookup = address.replace(/^e:/, '')
      return CONSTANTS[addressLookup] || address
    }).filter(address => address)
    toParts = [...new Set(toParts)]
    to = toParts.join(', ')
    ...
    const teamSubmission = {
      ...
      attachments: [
        ...
        '/api/submitter/json/default',
        '/api/submitter/csv/default'
      ]
    }
