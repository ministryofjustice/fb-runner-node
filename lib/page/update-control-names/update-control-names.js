...
const {instanceHasError} = require('~/fb-runner-node/page/validate-input/validate-input')
const redact = require('~/fb-runner-node/redact/redact')
const getComponentComposite = require('~/fb-runner-node/controller/component/get-component-composite')
...
const getRedactedValue = (nameInstance, userData, skipRedaction, scope) => {
  ...
  if (skipRedaction || value === undefined || value === null || String(value) === '') {
    return value
  }
  ...
  if (nameInstance.redact && !nameInstance.error) {
    ...
    const redacted = hasError ? false : nameInstance.redact
    if (redacted && value) {
