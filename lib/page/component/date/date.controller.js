  ...
  if (!errors.length) {
    if (instance.validation) {
      ...
      if (dateWithinNext) {
        ...
        if (!isWithinNext) {
          const targetValue = `${dateWithinNext.amount} ${dateWithinNext.unit}`
      ...
      if (dateWithinLast) {
        ...
        if (!isWithinLast) {
          const targetValue = `${dateWithinLast.amount} ${dateWithinLast.unit}`
...
DateController.getDisplayValue = (instance, userData) => {
  ...
  const partValues = parts.map(part => {
    const value = userData.getUserDataProperty(`${instance.name}-${part}`)
    if (value === undefined) {
      return
    }
    return {
