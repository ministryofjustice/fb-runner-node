...
const defaultFileUploadStringValues = {
...
const setFileUploadStringValues = (values) => {
  values = Object.assign(defaultFileUploadStringValues, values)
  return (id) => values[id]
}

const defaultFileUploadHint = `
${defaultFileUploadStringValues['fileupload.hint']}

${defaultFileUploadStringValues['fileupload.hint.timing']}
`.trim()
...
const setupUserDataStubs = (options = {}) => {
  ...
  getStringStub.callsFake(setFileUploadStringValues(strings))

  getInstancePropertyStub.callsFake(setFileUploadInstancePropertyValues(properties))
...
const getFileUploadInstances = async (pageInstance, userData) => {
  pageInstance = await setFileUploads(pageInstance, userData)
  const uploadControl = pageInstance.components[0]
...
const getPageInstanceWithFileUploadComponent = (props) => {
...
Object.keys(uploadTypes).forEach(uploadType => {
  test(`When setting up a ${uploadType} file upload and no files have been uploaded`, async t => {
    ...
    const pageInstance = getPageInstanceWithFileUploadComponent(uploadTypes[uploadType])

    const {uploadControl, uploadSlots} = await getFileUploadInstances(pageInstance, userData)
    const uploadSlot = uploadSlots[0]

    t.equal(uploadControl.label, 'Foo label', 'it should leave the label unchanged')
    t.equal(uploadControl.hint, defaultFileUploadHint, 'it should set the default hint')
...
test('When setting up a single file upload', async t => {
  ...
  const pageInstance = getPageInstanceWithFileUploadComponent()

  const {uploadControl, uploadSlots} = await getFileUploadInstances(pageInstance, userData)
  const uploadSlot = uploadSlots[0]

  t.equal(uploadSlot.label, 'Foo label', 'it should set the correct label for the upload slot')
  t.equal(uploadSlot.hint, defaultFileUploadHint, 'it should set the correct hint for the upload slot')
...
test('When setting up a multiple file upload and as many slots as are available are required', async t => {
  ...
  const {uploadControl} = await getFileUploadInstances(pageInstance, userData)
...
test('When a user asks for an additional upload slot and no slots already exist', async t => {
  ...
  const {uploadSlots} = await getFileUploadInstances(pageInstance, userData)
...
test('When setting up a file upload which has a file already uploaded', async t => {
  ...
  const {uploadControl, uploadSummary} = await getFileUploadInstances(pageInstance, userData)
...
const setupUserDataStubsWithFileUploadTypeHints = () => {
...
test('When setting up an upload with an accepts that is a type reference rather than a media-type', async t => {
  const {userData} = setupUserDataStubsWithFileUploadTypeHints()
