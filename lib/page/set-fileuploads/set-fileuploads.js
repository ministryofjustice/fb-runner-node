  ...
  uploadControls
    ...
    .forEach(control => {
      ...
      if (!control.hint && acceptHints) {
        acceptHints.forEach((type, index, array) => {
      ...
      const summaryList = currentFiles.map((file, index) => {
        ...
        const filetype = getString(`fileupload.file.type.${mimetypeKey}`) || file.mimetype.replace(/.*\//, '').replace(/\+.*/g, '')
      ...
      if (summaryList.length) {
        const heading = getString('fileupload.files.heading', userData.contentLang)
