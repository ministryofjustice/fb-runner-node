...
const userFileStoreClient = require('~/fb-runner-node/client/user-filestore/user-filestore')
...
const storeMOJUploadedFiles = proxyquire('./store-moj-uploaded-files', {
  fs: fs,
  '~/fb-runner-node/client/user-filestore/user-filestore': userFileStoreClient,
  '~/fb-runner-node/service-data/service-data': serviceData
})

test('When storeMOJUploadedFiles is passed a list of uploaded files to store', async t => {
  ...
  const fileErrors = await storeMOJUploadedFiles(userData, uploadedResults)
...
test('When storeMOJUploadedFiles is passed a files with policy properties', async t => {
  const uploadedResults = {
    files: {
      'test[1]': [
        {
  ...
  FBUserFileStoreClientStub.resetHistory()
  await storeMOJUploadedFiles(userData, uploadedResults)
...
test('When storeMOJUploadedFiles encounters an error uploading a file', async t => {
  const uploadedResults = {
    files: {
      'test[1]': [
        {
...
test('When storeMOJUploadedFiles finds a file in its list matches a slot marked for removal', async t => {
  const uploadedResults = {
    files: {
      'test[1]': [
        {
