...
const processMojUploadControls = proxyquire('./process-moj-upload-controls', {
  multer: multerStub
})

test('When processMojUploadControls is called', async t => {
  const userData = {
    req: {
      body: {}
    },
  ...
  const {files, fileErrors} = await processMojUploadControls(userData, uploadControls, allowedUploadControls)
...
test('When removing a file', async t => {
  ...
  const allowedUploadControls = [{fieldname: 'test[1]', maxCount: 1}]
  await processMojUploadControls(userData, uploadControls, allowedUploadControls)
