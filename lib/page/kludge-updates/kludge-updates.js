  ...
  let kludgedPageInstance = produce(pageInstance, draft => {
    ...
    if (draft._type === 'page.singlequestion' || draft._singlequestion === true) {
      ...
      if (question._type === 'group') {
        // TODO: REVISIT THIS KLUDGE
        const questionComponent = question.components && question.components[0] ? question.components[0] : undefined
        let questionId = questionComponent._id
        if (questionId) {
          questionId = questionId.replace(/--group/, '')
        }
