...
const {setWidthClass} = require('@ministryofjustice/fb-nunjucks-helpers')
const {format} = require('../../format/format')

const sizeElementMappings = require('../../element-size-mappings/element-size-mappings')
...
const kludgeUpdates = (pageInstance, userData) => {
  ...
  if (jp.query(pageInstance, '$..[?(@._type === "autocomplete")]').length) {
  ...
  const tableNodes = [].concat(jp.nodes(pageInstance, '$..[?(@._type === "table")].head[*]'))
    .concat(jp.nodes(pageInstance, '$..[?(@._type === "table")].rows[*][*]'))
    .map(item => item.value)
  ...
  jp.apply(pageInstance, '$..[?(@._type === "table")]', val => {
  ...
  // Apply cell widths
  jp.apply(pageInstance, '$..[?(@._type === "cell")]', val => {
