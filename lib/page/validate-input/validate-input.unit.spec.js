...
// since the pageInstance passed is updated in place
// this function allows for checking what should be passed to the validation methods
const normalisePageInstance = (pageInstance) => {
  const mormalisedPageInstance = deepClone(pageInstance)
  delete mormalisedPageInstance.$validated
  delete mormalisedPageInstance.errorList
  delete mormalisedPageInstance.errorTitle
  delete mormalisedPageInstance.$hasErrors
  mormalisedPageInstance.components.forEach(component => {
    if (component.validation && !Object.keys(component.validation).length) {
      delete component.validation
    }
    if (component.errors && !Object.keys(component.errors).length) {
      delete component.errors
    }
    component.items.forEach(item => {
      if (item.validation && !Object.keys(item.validation).length) {
...
test('When validating a page instance that has no errors', t => {
  ...
  const requiredPageInstance = normalisePageInstance(validateRequiredArgs[1])
  t.deepEqual(validateRequiredArgs[0][0]._id, 'b', 'it should pass the correct named instances to validateRequired method')
  t.deepEqual(requiredPageInstance, pageInstance, 'it should pass the pageInstance to validateRequired method')
  t.deepEqual(validateRequiredArgs[2], userData, 'it should pass the userData methods to validateRequired method')

  const validateValueArgs = validateValueSpy.getCall(0).args
  t.deepEqual(validateValueArgs[0][0]._id, 'b', 'it should pass the correct named instances to validateValue method')
...
test('When validating a page instance that has no errors', t => {
  ...
  const setErrorsPageInstance = normalisePageInstance(setErrorsArgs[0])

  t.deepEqual(setErrorsPageInstance, pageInstance, 'it should pass the pageInstance to the setErrors method')
