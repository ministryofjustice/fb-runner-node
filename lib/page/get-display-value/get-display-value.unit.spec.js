...
const resetStubs = () => {
  ...
  getComponentControllerStub.resetHistory()
...
test('When there is no name', t => {
  ...
  getDisplayValue(userData, nameInstance)
...
test('When the instance contains uploads', t => {
  resetStubs()
  ...
  const displayValue = getDisplayValue(userData, nameInstance)
  t.equals(displayValue, 'foo (23 kb)\n\nbar (48 kb)', 'it returns all the upload names and sizes')
  t.ok(formatStub.calledOnce, 'it calls the component\'s `getDisplayValue` method once')
  t.deepEqual(formatStub.getCall(0).args, ['foo (23 kb)\n\nbar (48 kb)', {}, {
    substitution: true,
    markdown: true,
