...
const setMOJFileUploadInstancePropertyValues = (values) => {
...
const defaultMOJFileUploadStringValues = {
  'mojFileupload.file.button.add': 'Add another file',
  'mojFileupload.file.button.remove': 'Remove button',
  'mojFileupload.file.description': 'Filename:{filename} Filetype:{filetype} Size:{size}',
  'mojFileupload.files.heading': '{count, select, 1{Already uploaded one} other{Already uploaded multiple}}',
  'mojFileupload.hint': 'default hint string',
  'mojFileupload.hint.timing': 'default timing string',
  'mojFileupload.hint.type.bar': 'Bar',
  'mojFileupload.hint.type.snack': 'Snax',
  'mojFileupload.hint.type.snack.falafel': 'Falafelly goodness',
  'mojFileupload.slot.button.remove': 'Remove {label}',
  'mojFileupload.slot.label': 'Slot label {count}'
}

const setMOJFileUploadStringValues = (values) => {
  values = Object.assign(defaultMOJFileUploadStringValues, values)
  return (id) => values[id]
}

const defaultMOJFileUploadHint = `${defaultMOJFileUploadStringValues['mojFileupload.hint']}\n\n${defaultMOJFileUploadStringValues['mojFileupload.hint.timing']}`
...
const setupUserDataStubs = (options = {}) => {
  ...
  getStringStub.callsFake(setMOJFileUploadStringValues(strings))

  getInstancePropertyStub.callsFake(setMOJFileUploadInstancePropertyValues(properties))
...
const getMOJFileUploadInstances = async (pageInstance, userData) => {
...
const getPageInstanceWithMOJFileUploadComponent = (props) => {
...
Object.keys(uploadTypes).forEach(uploadType => {
  test(`When setting up a ${uploadType} MOJ file upload and no files have been uploaded`, async t => {
    ...
    const pageInstance = getPageInstanceWithMOJFileUploadComponent(uploadTypes[uploadType])

    const {uploadControl, uploadSlots} = await getMOJFileUploadInstances(pageInstance, userData)
    const uploadSlot = uploadSlots[0]

    t.equal(uploadControl.label, 'Foo label', 'it should leave the label unchanged')
    t.equal(uploadControl.hint, defaultMOJFileUploadHint, 'it should set the default hint')
...
test('When setting up a single MOJ file upload', async t => {
  ...
  const pageInstance = getPageInstanceWithMOJFileUploadComponent()

  const {uploadControl, uploadSlots} = await getMOJFileUploadInstances(pageInstance, userData)
  const uploadSlot = uploadSlots[0]

  t.equal(uploadSlot.label, 'Foo label', 'it should set the correct label for the upload slot')
  t.equal(uploadSlot.hint, defaultMOJFileUploadHint, 'it should set the correct hint for the upload slot')
...
test('When setting up a multiple MOJ file upload and as many slots as are available are required', async t => {
  ...
  const {uploadControl} = await getMOJFileUploadInstances(pageInstance, userData)
...
test('When a user asks for an additional upload slot and no slots already exist', async t => {
  ...
  const {uploadSlots} = await getMOJFileUploadInstances(pageInstance, userData)
...
test('When setting up a MOJ file upload which has a file already uploaded', async t => {
  ...
  const {uploadControl, uploadSummary} = await getMOJFileUploadInstances(pageInstance, userData)
...
const setupUserDataStubsWithMOJFileUploadTypeHints = () => {
  return setupUserDataStubs({
    strings: {
      'mojFileupload.hint': '{types, or}',
      'mojFileupload.hint.timing': ''
    }
  })
}

test('When setting up an MOJ upload with an accepts that is a type reference rather than a media-type', async t => {
  const {userData} = setupUserDataStubsWithMOJFileUploadTypeHints()
