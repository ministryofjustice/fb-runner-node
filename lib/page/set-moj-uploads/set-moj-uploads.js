    ...
    .forEach(control => {
      ...
      if (!control.hint && acceptHints) {
        acceptHints.forEach((type, index, arr) => {
          ...
          let hintValue = getString(`mojFileupload.hint.type.${typeLookup}`, userData.contentLang)
          if (!hintValue) {
            ...
            hintValue = getString(`mojFileupload.hint.type.${typeLookup}`, userData.contentLang) || typeLookup
          }
          arr[index] = hintValue
        })
      ...
      if (maxAvailableSlots) {
        ...
        if (slots < maxAvailableSlots) {
          control.addFile = getString('mojFileupload.file.button.add', userData.contentLang)
        }
      }

      if (!control.hint) {
        let controlHintString = getString('mojFileupload.hint', userData.contentLang)
      ...
      if (slots) {
        let uploadExplanation = getString('mojFileupload.hint.timing', userData.contentLang)
      ...
      const fileDescription = getString('mojFileupload.file.description', userData.contentLang)
      const fileRemoveText = getString('mojFileupload.file.button.remove', userData.contentLang)
      const slotLabel = getString('mojFileupload.slot.label', userData.contentLang)
      const removeSlotText = getString('mojFileupload.slot.button.remove', userData.contentLang)
      ...
      const summaryList = currentFiles.map((file, index) => {
        ...
        const filetype = getString(`mojFileupload.file.type.${mimetypeKey}`) || file.mimetype.replace(/.*\//, '').replace(/\+.*/g, '')
      ...
      if (summaryList.length) {
        const heading = getString('mojFileupload.files.heading', userData.contentLang)
