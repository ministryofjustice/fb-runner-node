...
const processEnv = {
  ...
  USER_DATASTORE_URL: 'http://fb-user-data-store-api-svc-foo-bar.formbuilder-platform-foo-bar/',
...
const getEnvStub = stubModule('getEnv', () => processEnv)
...
test('When the USER_DATASTORE_URL ENV variable is set', t => {
  t.equal(constants.USER_DATASTORE_URL, 'http://fb-user-data-store-api-svc-foo-bar.formbuilder-platform-foo-bar/', 'constants should set USER_DATASTORE_URL to that value')
  t.end()
})

test('When the USER_DATASTORE_URL ENV variable is set and no value is passed for SAVE_RETURN_URL', t => {
  t.equal(constants.SAVE_RETURN_URL, 'http://fb-user-data-store-api-svc-foo-bar.formbuilder-platform-foo-bar/', 'constants should set SAVE_RETURN_URL to that value')
  t.end()
})
