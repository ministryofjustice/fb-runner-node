...
const resolveNestedStrings = (value, args, options = {}) => {
  ...
  value = value.replace(/\[%.+?%\]/g, (str) => {
    ...
    const [aliasNameUrlType, aliasNameUrl] = aliasName.split(':')
    if (aliasNameUrl) {
      if (!['url', 'fqdn'].includes(aliasNameUrlType)) {
        return value
      }
      aliasName = aliasNameUrl
      aliasProperty = 'url'
    }
    ...
    if (aliasProperty === 'url') {
      str = getUrl(aliasName, parsedArgs, embeddedOptions.lang)
      if (aliasNameUrlType === 'fqdn') {
        str = getFullyQualifiedUrl(str)
      }
    } else {
      aliasProperty = aliasProperty || 'value'
      str = getInstanceLangProperty(aliasName, aliasProperty, embeddedOptions.lang)
      if (str) {
