require('@ministryofjustice/module-alias/register') // require('resolve-pkg')('@ministryofjustice/fb-runner-node', {cwd: __dirname}))
...
const {getUrl, getFullyQualifiedUrl} = require('~/fb-runner-node/route/route')
...
const resolveNestedStrings = (value, args, options = {}) => {
  ...
  value = value.replace(/\[%.+?%\]/g, (str) => {
    ...
    const [nestedStr, ...argParts] = str.split(' ')
    ...
    for (const nestedStr of nestedStrs) {
