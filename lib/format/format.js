...
const resolveNestedStrings = (value, args, options = {}) => {
  ...
  value = value.replace(/\[%.+?%\]/g, (str) => {
    ...
    for (let index = 0; index < nestedStrs.length; index++) {
      str = resolveNestedString(nestedStrs[index], value, args, parsedArgs, embeddedOptions)
      if (str) {
        return str
      }
    }
...
`
  ...
  if (markdown) {
    // // backref so it's the same char repeated
    // if (formattedValue && formattedValue.match(/^(#|~|\*)+$/)) {
    //   return formattedValue
    // }
    // TODO: cache output?
    formattedValue = Markdown(formattedValue).trim()
