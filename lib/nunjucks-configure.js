...
const nunjucksConfigure = (app, appDir, kitDir, options = {}) => {
  const appViews = [
    ...
    path.join(appDir, 'node_modules', 'fb-specification', 'specifications'),
    path.join(appDir, 'node_modules', '@govuk-frontend', 'frontend', 'components'),
    path.join(appDir, 'node_modules', '@govuk-frontend', 'frontend')
  ]
  ...
  const specsPath = path.join(nodeModules, 'fb-specification', 'specifications')
  const fbMacroPaths = glob.sync(`${specsPath}/**/*.njk`)

  const govukPath = path.join(nodeModules, '@govuk-frontend', 'frontend', 'components')
